...
Эта таблица **появляется автоматически** в момент самой первой инициализации СУБД. Когда вы запускаете чистый контейнер Docker, сервер MySQL сам создает служебные базы данных и таблицы для своих внутренних нужд.

### 3. Что означает точка в записи `mysql.user`?
...
> [Вставьте сюда скриншот списка пользователей, где в таблице результатов четко видна строка с sys_temp]
